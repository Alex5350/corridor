...
| Authorization rules scattered per app cannot survive a claim-shape change | Central XACML PDP with file-based policies and default deny, enforced by a portal PEP at the API boundary that fails closed and caches decisions | Rules become one reviewable artifact instead of three codebases, and the portal proves it enforces them | Who-can-do-what is answerable, and auditable, from `policies/`, with 403 `cor:PdpDenied` on every deny | [ADR 0007](docs/adr/0007-xacml-central-pdp.md), `Xacml/PdpEngine.cs`, `Auth/Pdp/PdpHttpClient.cs`, `policies/*.xacml.xml` |
...
| Portal unit (`tests/Corridor.Portal.Tests`) | 63 tests | ACS endpoint, login routing, claims transformation, problem-details mapping, TrustMode service, checklist logic, PDP enforcement (permit, deny, fail closed, cache) |
...
| Integration (`tests/Corridor.IntegrationTests`) | 31 tests; Testcontainers SQL + real child processes on contract ports | OIDC end to end, SAML end to end, direct SOAP, the portal bridge, SCIM provisioning, XACML, portal PEP enforcement, the admin flip, status transitions, service health |
